...
def central_difference(f: Any, *vals: Any, arg: int = 0, epsilon: float = 1e-6) -> Any:
    ...
    """
    new_vals = type(vals)([vals[i] + epsilon if i == arg else vals[i] for i in range(len(vals))])
...
def topological_sort(variable: Variable) -> Iterable[Variable]:
    ...
    def dfs(node: Variable):
        if (node.unique_id in was) or node.is_constant():
            return
        for parent in node.parents:
            dfs(parent)
        was.add(node.unique_id)
        result.append(node)
...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    for node in top_sort:
        ...
        if not(node.is_leaf()):
            for var, d in node.chain_rule(derivative):
